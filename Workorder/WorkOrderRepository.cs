...
namespace SIMTech.APS.WorkOrder.API.Repository
{
    ...
    public class WorkOrderRepository : Repository<WorkOrder>,  IWorkOrderRepository
    {
        ...
        public async Task<List<WIP1>> GetWIPs()
        {
            ...
            try
            {
                result = await _dbContext.Database.ExecuteSqlRawAsync("sp_GetWIP");
            }
            catch (Exception e)
            {
                Console.WriteLine("Error in  sp_UpdateWorkOrderStatus" + $"{ e.Message}");
            ...
            return await _dbContext.WIPs.Where(x => x.Id > 0).ToListAsync();
        }
    }
}
