...
namespace SIMTech.APS.Inventory.API.Controllers
{
    ...
    public class InventoryController : ControllerBase
    {
        ...
        public IActionResult AddRawMaterial([FromBody] InventoryPM inventoryPM)
        {
            ...
            string poNumber = "";
            int lineNo = 1;
            int n =inventoryPM.Remarks.IndexOf("salesorder :");
            if (n>0)
            {
                var so=inventoryPM.Remarks.Substring(n +12);
                int k = so.IndexOf("/");
                poNumber = (so.Substring(0, k)).Trim ();
                lineNo = int.Parse ( so.Substring(k + 1, so.Length -k-1));
            }


            AllocateRawMaterial(inventory.ProductId,poNumber,lineNo);
        ...
        [HttpPost("MaterialDeallocation")]
        [Consumes(MediaTypeNames.Application.Json)]
        [ProducesResponseType(StatusCodes.Status409Conflict)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<int>> DeallocateRawMaterials([FromBody] int workOrderId)
        {
            var n =await _inventoryRepository.DeallocateRawMaterial(workOrderId);
            return Ok(n);
        }
        ...
        private void AllocateRawMaterial(int materialId, string poNumber, int lineNo)
        {
            ...
            if (inventory != null)
            {
                ...
                if (balanceQuantity > 0)
                {
                    ...
                    var workOrderRawmaterials = ApiGetWorkOrderMaterialShortages1(materialId,poNumber ,lineNo );
        ...
        private List<IdNamePM> ApiGetWorkOrderMaterialShortages1(int materialId, string poNumber , int lineNo )
        {
            var items = new List<IdNamePM>();
            var apiBaseUrl = Environment.GetEnvironmentVariable("RPS_WORKORDER_URL");
            ...
            if (!string.IsNullOrWhiteSpace(apiBaseUrl))
            {
                try
                {
                    ...
                    var result = HttpHelper.Get<List<IdNamePM>>(apiBaseUrl, $"MaterialShortage1/{materialId}/{poNumber}/{lineNo}");
