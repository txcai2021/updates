...
namespace SIMTech.APS.Integration.RabbitMQ
{
    ...
    public class SalesOrderConsumer : BackgroundService
    {
                ...
                if (salesOrderCS!=null)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        if (salesOrderCS.OrderItems.Where (x=>x.Priority == "VERYURGENT").Count() >0)
                        {
                            ...
                            var alert = new AlertPM() { Id = result, Title = "Very Urgent Order", AlertType = "UrgentOrder", Action = "Release", Description = description, CreatedBy = "OrderApp" };
        ...
        private int ApiAddSalesOrder(SalesOrderCS salesOrder)
        {
            ...
            var salesOrderPM = new SalesOrderPM() { SalesOrderNumber = salesOrder.OrderNo, OrderType = ESalesOrderType.StandardOrder, PurchaseOrderNumber = salesOrder.OrderNo, Status = 0, CustomerId = customerId, OrderDate = DateTime.Now, DueDate = DateTime.MaxValue };
            var cnt = 0;
            ...
            foreach (var lineItem in salesOrder.OrderItems)
            {
                ...
                var salesOrderLinePM = new SalesOrderLinePM() { Quantity = lineItem.Quantity, ProductId = lineItem.ProductId, LineNumber = lineItem.LineNumber ?? cnt, Status = ESalesOrderLineStatus.Pending, DueDate = lineItem.DueDate ?? salesOrderPM.OrderDate.AddDays(7), UrgentFlag = urgentFlag, Remarks=lineItem.PersonalizedMsg , Remarks1 = "Color: " + lineItem.Color + "Scent: " + lineItem.Scent };
                //if (salesOrderLinePM.UrgentFlag != EWorkOrderUrgentFlag.Standard) salesOrderLinePM.DueDate = ((DateTime)salesOrderLinePM.DueDate).AddHours(4);
                if (salesOrderLinePM.DueDate<salesOrderPM.OrderDate )
                {
                    //To handle timezone issue from OrderApp
                    salesOrderLinePM.DueDate = ((DateTime)salesOrderLinePM.DueDate).AddHours(8);
                    //to make sure the order due date is greater than order date
                    if (salesOrderLinePM.DueDate < salesOrderPM.OrderDate) salesOrderLinePM.DueDate = ((DateTime)salesOrderPM.OrderDate).AddHours(4);
                }
