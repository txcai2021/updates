...
namespace SIMTech.APS.Routing.API.Controllers
{
    ...
    public class RouteController : ControllerBase
    {
        ...
        [HttpGet("IdNames/{routeIds}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<IdNamePM>>> GetRouteIdNames(string routeIds)
        {
            ...
            if (routeIds == "0")
            {
                routes = (await _routeRepository.GetQueryAsync(x => x.Id>0)).Select(x => new BasePM() { Id = x.Id, Code = x.RouteName, Name = x.RouteName, Description = x.Description,Value =(x.Version??0).ToString () }).ToList();
            }
            else
            {
                try
                {
                    rIds = routeIds.Split(",").Select(x => Int32.Parse(x)).ToList();
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.Message);
                }
                routes = (await _routeRepository.GetQueryAsync(x => rIds.Contains(x.Id))).Select(x => new BasePM() { Id = x.Id, Code = x.RouteName, Name = x.RouteName, Description = x.Description, Value = (x.Version ?? 0).ToString() }).ToList();
            }
        ...
        public async Task<ActionResult<RoutePM>> UpdateRoute(int id, [FromBody] RoutePM routePM)
        {
            ...
            existingRoute.ModifiedBy = route.ModifiedBy;
            existingRoute.ModifiedOn = DateTime.Now;
        ...
        public async Task<ActionResult<int>> CopyRoute([FromBody] CopyRoute newRoute)
        {
            ...
            var route1 = await CopyRouteTemplate(route, newRoute.NewRouteName, newRoute.Description, newRoute.CreatedBy);          
            return Ok(route1.Id);
        }
        ...
        {           
            ...
            var copiedRoute = new Route() { RouteName = route.RouteName, Type = route.Type, Description = route.Description, Comment = route.Comment, Version=maxVersionNo + 1, IsActive = route.IsActive, IsDefault = route.IsDefault, LocationId = route.LocationId, TraceLevel = route.TraceLevel,CreatedOn=DateTime.Now };
        ...
        private async Task<Route> CopyRouteTemplate(Route route, string routeName, string description, string createdBy )
        {

            
            var copiedRoute = new Route() { RouteName = routeName, Type = route.Type, Description = description, Comment = route.Comment, Version = 1, IsActive = route.IsActive, IsDefault = route.IsDefault, LocationId = route.LocationId, TraceLevel = route.TraceLevel , CreatedBy=createdBy,CreatedOn=DateTime.Now};
            await _routeRepository.InsertAsync(copiedRoute);
