                ...
                if (inventoryPlan.RawMaterialRebalacingPlan!=null)
                {
                    foreach (var invPlan in inventoryPlan.RawMaterialRebalacingPlan)
                    {
                        ...
                        if (result > 0)
                        {
                            ...
                            Console.WriteLine("Sending inventory rebalance alerts");

                            var description = "Received raw material for " + material.PartNo +",Quantity"+material.CompletedQty+" "+  material.Remarks;

                            var alert = new AlertPM() { Id = 0, Title = "Inventory Rebalance", AlertType = "RawMaterial", Action = "Release", Description = description, CreatedBy = "IPS" };
