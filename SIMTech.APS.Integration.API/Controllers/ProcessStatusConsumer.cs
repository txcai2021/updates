...
namespace SIMTech.APS.Integration.RabbitMQ
{
    ...
    public class ProcessStatusConsumer : BackgroundService
    {
        ...
        private int ApiUpdateWorkOrderStatus(ProcessUpdate processUpdate)
        {
            ...
            Console.WriteLine("In ApiUpdateWorkOrderStatus/RPS_WorkOrder_URL:" + apiBaseUrl??"");
            ...
            Console.WriteLine("work order number:" + woStatus.Name + ",category:" + woStatus.Category + ",status:" + woStatus.String1 + ",currentOP:" + woStatus.String2 + ",completedQty:" + woStatus.Int1.ToString() + ",scrapQty:" + woStatus.Int2.ToString());
            ...
            if (!string.IsNullOrWhiteSpace(apiBaseUrl))
            {
                try
                {
                    Console.WriteLine("Before ApiUpdateWorkOrderStatus call");
                    var task = HttpHelper.PostAsync<IdNamePM, int>(apiBaseUrl, "Status", woStatus);
                    if (task!=null)
                    {
                        task.Wait();
                        result = task.Result;
                        Console.WriteLine("After calling API, status:"+result.ToString ());
                    }
