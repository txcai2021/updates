...
namespace SIMTech.APS.Scheduling.API.Controllers
{
    ...
    public class ScheduleController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public bool RunScheduler([FromBody] ScheduleParameters scheduleParameters)
        {
        ...
        public ActionResult<bool> RunDiSimLab(string filename)
        {
            ...
            foreach (var wo in scheduleDetails.Select(x=>x.WorkOrderNumber).Distinct().ToList ())
            {
        ...
        [HttpGet("RunGAP/{fileName}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public ActionResult<IEnumerable<ProductLotSize>> RunGAP(string fileName)
        {
           
            var lotSizes = ApiGetGAPProductLotSize();

            var productLotsizes = new List<BasePM>();

            foreach (var lotSize in lotSizes)
            {
                Console.WriteLine(lotSize.PartName+":"+lotSize.LotSize);
                var a = new BasePM() {Name=lotSize.PartName, Value=lotSize.LotSize.ToString () };
        ...
        private List<ProductLotSize> ApiGetGAPProductLotSize()
        {
