...
namespace SIMTech.APS.Product.API.Controllers
{
    ...
    public class ItemController : ControllerBase
    {
            ...
            else
            {
                ...
                try
                {
                    if (kitTypePM.ThumbnailImage != null && kitTypePM.ThumbnailImage.Length > 0)
                    {
                        ...
                        if (pictureId > 0)  kitType.PictureId = pictureId;
                    }
                }
                catch { }
        ...
        {
            var linkedPart = _exceptionManager.Process(() => _itemRepository.GetQuery(x=>x.Int2!=null && x.Int2 == partId).FirstOrDefault (), "ExceptionShielding");
        ...
        private KitTypePM GetCustomersForKitType(Item item)
        {
            ...
            try
            {
                ...
                {
                    var picture = ApiGetPicture(item.PictureId ?? 0);
                    if (picture != null)
                    {
        ...
        private Item CopyPart(int partId, string partName, string revision, bool disableOldRevison)
        {
            ...
            newPart.BillOfMaterialComponents = new List<BillOfMaterial>();
            foreach (var bom in item.BillOfMaterialComponents)
            {
                ...
                newPart.BillOfMaterialComponents.Add(bom1);
            }
