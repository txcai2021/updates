...
namespace SIMTech.APS.Scheduling.API.Controllers
{
    ...
    public class ScheduleController : ControllerBase
    {
        ...
        [HttpGet("LateJobs/{scheduleId}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<List<LateJob>>> GetLateJobs(int scheduleId)
        {

            var jobs = await _scheduleDetailRepository.GetQuery(x => x.ScheduleId == scheduleId && (x.ScheduleType == "X" || x.ScheduleType == "R") && x.EndDate > x.WorkOrderDueDate).ToListAsync();
            var lastJobs = jobs.GroupBy(x => x.WorkOrderId).Select(grp => new IdNamePM() { Id = grp.Key, Int1 = grp.Max(x => x.Sequence) }).ToList();

            var result = from j in jobs
                         join l in lastJobs on j.WorkOrderId equals l.Id
                         where j.Sequence ==l.Int1
                         select  new LateJob { WorkOrderId =j.WorkOrderId , WorkOrderNumber =j.WorkOrderNumber , DueDate =j.WorkOrderDueDate , LateOperation =j.OperationName , CompletionDate =j.EndDate, PartNo =j.ItemName };          

            return Ok(result.ToList ());
        }

        [HttpGet("UnassignedJobs/{scheduleId}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<List<LateJob>>> GetUnassignedJobs(int scheduleId)
        {
            var jobs = await _scheduleDetailRepository.GetQuery(x => x.ScheduleId == scheduleId && x.ScheduleType == "U" ).
                Select (j=> new LateJob { WorkOrderId = j.WorkOrderId, WorkOrderNumber = j.WorkOrderNumber, DueDate = j.WorkOrderDueDate, LateOperation = j.OperationName,  PartNo = j.ItemName, Remarks=j.MaxString1,Quantity =j.Quantity }).ToListAsync();
    
            return Ok(jobs);
        }

        [HttpGet("MachineBreakdown/{machineName}/{startDate}/{endDate}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<List<string>>> GetAffectedWorkOrders(string machineName, DateTime startDate, DateTime endDate)
        {
            var job = _scheduleDetailRepository.GetQuery(x => x.EquipmentName == machineName && x.ScheduleType == "x" && x.StartDate < endDate && x.EndDate > startDate).OrderBy(x => x.WorkOrderNumber).Select(x => x.WorkOrderNumber).Distinct().ToList();
        ...
        [HttpPost("AutoRunSchedule/{unit}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> AutoRunScheduler(string unit)
        {
            var locationName = unit;

            var st = DateTime.Now;
            var startDate1 = ApiGetOptionSettingByName("DPS_ScheduleStartDate");
            if (DateTime.TryParse(startDate1, out DateTime st1)) st = st1;

            var duration = 2;
            var duration1 = ApiGetOptionSettingByName("DPS_ScheduleWorkingDays");
            if (Int32.TryParse(duration1,out int n)) duration = n;

            var startDate = new DateTime(st.Year, st.Month, st.Day, st.Hour, st.Minute,0 );
            var endDate = startDate.AddDays(duration);
            var dispatchRule = GetSchedulingDispatchRules().First().Name;
            var objective =GetSchedulingObjectives().First().Name ;
            var wip = false;
            var command = false;
        ...
        public async Task<IActionResult> ConfirmSchedule(int scheduleId)
        {
            ...
            var ppResult =GeneratePPOrderRoute(scheduleId);

            if (ppResult == -1)  return StatusCode(StatusCodes.Status503ServiceUnavailable, ppResult);
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                var a = Newtonsoft.Json.JsonConvert.SerializeObject(schedulePM);
                var body = Encoding.Default.GetBytes(a);
             
                Console.WriteLine("before publish:" + a);
        ...
        private int GeneratePPOrderRoute(int scheduleId)
        {
          ...
          //                UPDATE Integration.PPOrderRoute SET materialIdList = @MaterialIds WHERE PPID = @PPOrderID and SeqNo = @Sequence
            ...
            var result =ApiInsertPPOrderRoute(pporderRoutes);
            var n = 0;
            //try to rerun api at most 3 times to add integratin data if conneciton refused
            while (result ==-1 && n<3)
            {
                n++;
                Task.Delay(2000);
                Console.WriteLine("rerun ApiInsertPPOrderRoute: "+n.ToString ());
                result = ApiInsertPPOrderRoute(pporderRoutes);
            }
        ...
        private int ApiInsertPPOrderRoute(List<PporderRoute> ppRoutes)
        {
            ...
            Console.WriteLine("ApiInsertPPOrderRoutepp:"+ppRoutes.Count().ToString ());
            ...
            if (!string.IsNullOrWhiteSpace(apiBaseUrl))
            {
                ...
                catch (Exception e)
                {
                    Console.WriteLine(e.Message);
                    if (e.InnerException != null) Console.WriteLine(e.InnerException.Message);
                    if (e.InnerException.Message.Contains ("Connection refused")) result = -1;
